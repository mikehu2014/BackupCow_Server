...
type
  ...
  PChrArray = ^TChrArray;
  TChrArray = array[0 .. MAXINT -1] of ansiChar;
...
begin
  ...
  pansichar(DiagF) := pansichar(DiagBufferF) - sizeof(integer)*(MAX_DIAGONAL-(len1+1));
  for i := - (len1+1) to (len2+1) do DiagF[i] := -MAXINT;
  DiagF[1] := -1;

  pansichar(DiagB) := pansichar(DiagBufferB) - sizeof(integer)*(MAX_DIAGONAL-(len1+1));
